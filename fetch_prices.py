...
def from_yahoo():
    ...
    for t, c in zip(ts, closes):
        ...
        out.append({"d": d, "c": round(float(c), 4)})
    # 야후의 일봉이 하루 늦게 들어오는 일이 있어, 장이 끝났으면
    # meta 의 최신 체결가로 마지막 하루를 보충한다.
    meta = res.get("meta") or {}
    px, tm = meta.get("regularMarketPrice"), meta.get("regularMarketTime")
    state = (meta.get("marketState") or "").upper()
    if px and tm and state in ("CLOSED", "POST", "POSTPOST", "PREPRE", "PRE"):
        d = datetime.datetime.utcfromtimestamp(int(tm)).strftime("%Y-%m-%d")
        if d not in [x["d"] for x in out]:
            out.append({"d": d, "c": round(float(px), 4)})
            print("  + meta 로 %s $%s 보충" % (d, round(float(px), 4)), file=sys.stderr)
    if len(out) < 20:
